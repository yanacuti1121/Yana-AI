...
| --- | --- | --- |
| [OpenHands](https://github.com/All-Hands-AI/OpenHands) (now "Agent Canvas" — its own docs say the UI is "not responsible for... executing agent actions" or "providing the sandbox"; that layer lives in [`OpenHands/software-agent-sdk`](https://github.com/OpenHands/software-agent-sdk)) | A self-hosted control center orchestrating multiple coding-agent backends (Claude Code, Codex, Gemini, any ACP agent) | Agent-orchestration-centric — a control center for running agents, not an authority calculus for what any one of them may execute |
...
| [Goose](https://github.com/aaif-goose/goose) | Provider-agnostic, multi-provider, multi-extension (MCP) agent runtime cohesion — closest to Yana's provider plane | A capable agent runtime first; execution approval and capability scoping are not its organizing principle the way authority is Yana's |
