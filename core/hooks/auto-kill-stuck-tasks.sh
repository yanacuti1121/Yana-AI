...
# when YAMTAM_AUTO_KILL=1 is set. This avoids killing legitimate user processes.

set -uo pipefail

THRESHOLD_SECONDS="${YAMTAM_STUCK_THRESHOLD_SECONDS:-3600}"
...
ps -eo pid=,etimes=,args= 2>/dev/null | while read -r pid etimes cmd; do
  ...
  if (( etimes > THRESHOLD_SECONDS )) && [[ "$cmd" =~ (playwright|chrome-headless|chromium|npm[[:space:]]+run[[:space:]]+test:e2e|pnpm.*test:e2e) ]]; then
    ...
    if [[ "${YAMTAM_AUTO_KILL:-0}" == "1" ]]; then
      ...
      printf '{"decision":"warn","reason":"Stuck Task Monitor: likely stuck process detected, not killed. Set YAMTAM_AUTO_KILL=1 to allow kill. Log: .claude/state/stuck-tasks.log"}\n'
    fi
    exit 0
  fi
done
