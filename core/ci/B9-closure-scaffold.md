...
| 10 | Required-check recommendation | **DONE, and acted on** | B0 recommended; branch protection was actually enabled this session with all 6 `ci.yml` jobs REQUIRED FOR MERGE, including Dependency Audit (verified live via `gh api`) — not just a recommendation left on paper |
...
| 14 | Supply-chain gates | **DONE** | cargo-audit + pip-audit + npm-audit wired and REQUIRED FOR MERGE (B1); 3 known CVEs formalized in an exceptions registry with owner/created/review-by fields (B5) |
