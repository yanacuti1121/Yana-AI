...
sync_target() {
  python3 core/scripts/sync_codex.py --target "$1"
}

sync_fixture() {
  FIXTURE_SOURCE="$1" FIXTURE_TARGET="$2" python3 - "$REPO_ROOT" <<'PY'
import os
import sys
...
sync_codex(Path(os.environ["FIXTURE_TARGET"]), source_root=Path(os.environ["FIXTURE_SOURCE"]))
PY
}

sync_target "$TARGET_A"
...
python3 core/scripts/sync_codex.py --check --target "$TARGET_A"
echo "PASS: generated target validates"

SECOND_SYNC_OUTPUT="$(sync_target "$TARGET_A")"
grep -Fq 'Codex sync: 0 agent files, 0 command adapters' <<<"$SECOND_SYNC_OUTPUT"
echo "PASS: second generation produces zero changes"

sync_target "$TARGET_B"
diff -qr "$TARGET_A" "$TARGET_B"
echo "PASS: independent targets are deterministic"

printf '\n' >> "$TARGET_A/.codex/agents/mail-reader.toml"
if python3 core/scripts/sync_codex.py --check --target "$TARGET_A" >"$TEMP_ROOT/stale.out" 2>&1; then
...
if python3 core/scripts/sync_codex.py --check --target "$MISSING_TARGET" >"$TEMP_ROOT/missing.out" 2>&1; then
...
Path safety fixture.
EOF
sync_fixture "$PATH_SOURCE" "$PATH_TARGET"
...
EOF
if sync_fixture "$COLLISION_SOURCE" "$COLLISION_TARGET" >"$TEMP_ROOT/collision.out" 2>&1; then
