...
| --- | --- | --- |
| [OpenHands](https://github.com/All-Hands-AI/OpenHands)（现已更名为 "Agent Canvas" —— 官方文档自己写明这个 UI "不负责……执行代理动作"或"提供 sandbox"；那一层在 [`OpenHands/software-agent-sdk`](https://github.com/OpenHands/software-agent-sdk)） | 一个自托管的控制中心，编排多个 coding-agent 后端（Claude Code、Codex、Gemini、任何 ACP agent） | 以代理编排为中心 —— 是一个运行代理的控制中心，而不是决定某个代理能执行什么的权限演算 |
...
| [Goose](https://github.com/aaif-goose/goose) | Provider 无关、多 provider、多扩展（MCP）——与 Yana 的 provider plane 最接近 | 首先是一个强大的 agent runtime；执行审批与 capability 范围划定并不是它的组织原则，不像权限之于 Yana |
