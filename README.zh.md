...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/📜_规则-69-b96b80?style=flat-square" />
  <img src="https://img.shields.io/badge/🪝_钩子-55-b78f3d?style=flat-square" />
...
**Yana AI** 是面向 AI 编程工具的个人智能体操作系统 — 包含运行时安全钩子、分层记忆系统、101 个专家智能体、1,989 个技能,以及一个在危险 AI 操作执行前进行拦截的 Rust 运行时。

可与 **Claude Code**、**Cursor**、**Windsurf**、**Antigravity**、**Kiro**、**OpenCode**、**Zed**、**Gemini**、**GitHub Copilot**、**Aider** 等工具配合使用。


> **v0.43.0 新功能：** **yana-rt 现在可在浏览器中运行** — 通过 wasm-bindgen 构建的 WebAssembly 将危险命令拦截守卫带到浏览器扩展、VS Code 和 Node.js（`npm install yana-rt`）。发布流水线已全面自动化 — 推送 tag 时自动发布至 npm、PyPI 和 crates.io。智能体数量已核准为 101 个。新增规则 70：上下文忠实法则 — 用户提供的数据优先于模型训练数据。
>
> **2026-07-01 更正：** 上面这条"核准为 101 个"的说明本身也是错的（旧数字 162 与仓库里任何其他文件都对不上——ARCHITECTURE.md 写的是 95，文档站的实际数据是 93）。本页所有数字现在都由 `core/scripts/generate-stats.py` 直接从文件系统生成，不再手工填写。
...
> **101 个智能体是什么意思？** 它们并不是同时运行的 101 个 AI 模型 — 而是预先定义好的专家角色(安全、前端、后端、测试、学习、日常助理……),用于路由和任务组织。在正常使用中,只会激活当前任务所需的那一个智能体;大多数请求只用到单一模型与单一智能体路径。
>由仓库指标自动生成
最后更新：2026-06-30
---
...
```
core/
├── hooks/          # 50 个 PreToolUse / PostToolUse / Stop 钩子
├── rules/          # 69 条强制规则（安全、正确性、UI、git）
...
- Rust 运行时（`yana-rt`）、101 个智能体、1,989 个技能、多工具适配支持
- 15 个工具适配层（Claude Code、Cursor、Windsurf、Antigravity、Kiro、Zed、Gemini、Copilot、Aider……）

这 1,989 个技能覆盖：前端、后端、AI/LLM、安全、Kubernetes、WebAssembly、DevOps、数据库、测试等等。另有两个面向非编程场景的全新智能体角色：学习（`hoc-tap`）与日常生产力（`daily-assistant`）。
...
- **skill** → 与 1,989 条技能索引匹配,派发到准确的技能智能体
