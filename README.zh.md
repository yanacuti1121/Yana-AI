...
> **v0.42.3 新功能：** **yana-rt 现在可在浏览器中运行** — 通过 wasm-bindgen 构建的 WebAssembly 将危险命令拦截守卫带到浏览器扩展、VS Code 和 Node.js（`npm install yana-rt`）。发布流水线已全面自动化 — 推送 tag 时自动发布至 npm、PyPI 和 crates.io。智能体数量已核准为 101 个。新增规则 70：上下文忠实法则 — 用户提供的数据优先于模型训练数据。
>
