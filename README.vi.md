...
├── config/
│   ├── core-lock.json    # manifest SHA-256 — pin 278 file core
...
- **Implementation SSRF và supply-chain có tồn tại, nhưng mức bảo vệ phụ thuộc runtime surface.** Chain Claude/Codex cục bộ của repository không đăng ký `tool-validator.sh`, `dependency-safety-gate.sh` hoặc `supply-chain-guard.sh`. Manifest Claude plugin đăng ký hai hook supply-chain nhưng không đăng ký `tool-validator.sh`. Vì vậy không được tuyên bố chặn metadata endpoint hay typosquat nếu chưa kiểm tra bề mặt cài đặt đang hoạt động. Evidence execution-path sinh tự động nằm tại `docs/operations/hook-execution-path-audit.md`.
