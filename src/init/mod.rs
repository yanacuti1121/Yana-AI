...
#[derive(Subcommand)]
pub enum InitAction {
    /// Initialize YAMTAM in a project directory
    Run {
...
# YAMTAM pre-tool-use gate — generated by yamtam-rt init
...
for pattern in "${BLOCKED[@]}"; do
  if echo "$CMD" | grep -qE "$pattern" 2>/dev/null; then
    echo '{"decision":"block","reason":"[yamtam] Blocked pattern: '"$pattern"'"}'
    exit 0
  fi
done
...
const YAMTAM_CONFIG: &str = r#"# .yamtam/config.toml — generated by yamtam-rt init
version = "1.0"
...
fn file_specs() -> Vec<FileSpec> {
    vec![
        ...
        FileSpec { path: ".yamtam/config.toml",          content: YAMTAM_CONFIG,    exec: false },
    ]
}
...
fn run(dir: &str, _yes: bool, dry: bool) {
    ...
    println!("  yamtam-rt init{}", if dry { " (dry run)" } else { "" });
    ...
    if !dry {
        println!();
        println!("  Next steps:");
        println!("    1. npm install yamtam-engine   # wire hooks");
        println!("    2. npx yamtam-install           # activate");
        println!("    3. yamtam-rt scan .             # first scan");
        println!("    4. yamtam-rt doctor run .       # health check");
    }
}
