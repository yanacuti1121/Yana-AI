    ...
    for f in findings {
        *by_category.entry(f.category.clone()).or_insert(0) += 1;
        let entry =
            rule_counts
                .entry(f.id.clone())
                .or_insert((f.severity.clone(), f.category.clone(), 0));
        entry.2 += 1;
    }
    let mut top_rules: Vec<TopRule> = rule_counts
        .into_iter()
        .map(|(id, (severity, category, count))| TopRule {
            id,
            severity,
            count,
            category,
...
pub fn run_audit(
    ...
    let ignore_matcher = files::PathMatcher::new(&ignore_patterns);
    let inventory = files::FileInventory::new(target);
    ...
    // Canonicalize target so strip_prefix works correctly against the absolute
    // paths returned by resolve_files().
    let canon_target =
    ...
    for rule_set in &rule_sets {
        ...
        for check in &rule_set.checks {
            ...
            if specific_target.is_empty() {
                continue;
            }
            let clean = specific_target
                .trim_start_matches("./")
                .trim_start_matches('/');
            ...
            let rel = explicit_path
                .strip_prefix(&canon_target)
                .unwrap_or(&explicit_path)
                .to_string_lossy()
                .to_string();
            if let Some(df) = diff_files {
                if !df.contains(&rel) {
                    files_skipped += 1;
                    continue;
                }
            }
            if ignore_matcher.matches_path_or_basename(&rel) {
                files_ignored += 1;
                continue;
            }
        ...
        let exclude_matcher = files::PathMatcher::new(&effective_excludes);
        ...
        if !effective_patterns.is_empty() {
            ...
            let target_files = inventory.resolve(&effective_patterns, &[]);
            for fp in target_files {
                ...
                if exclude_matcher.matches_path_or_basename(&rel) {
                    files_ignored += 1;
                    continue;
                }
                let content = match files::read_file_safe(&fp) {
                    ...
                    None => {
                        all_findings.push(Finding {
                            id: "SCAN_SKIP".into(),
                            severity: "INFO".into(),
                            category: rule_set.scope.clone(),
                            file: rel.clone(),
                            line: None,
                            rule: "scanner/file-unreadable".into(),
                ...
                for check in &rule_set.checks {
                    ...
                    if check.get("target").and_then(|v| v.as_str()).is_some() {
                        continue;
                    }
    ...
    let status = if all_findings.iter().any(|f| f.severity != "INFO") {
        "findings"
    } else {
        "clean"
    };
...
fn run_check(
    ...
    if let Some(specific) = check.get("target").and_then(|v| v.as_str()) {
        ...
        if rel != clean && !rel.ends_with(clean) {
            return vec![];
        }
    }
