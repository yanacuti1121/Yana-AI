...
pub fn render_console(report: &ScanReport, no_color: bool, quiet: bool) -> String {
    ...
    lines.push(c("1;36", "┌─────────────────────────────────────────────────────┐"));
    lines.push(c("1;36", "│  YAMTAM Agent Audit Report                          │"));
    lines.push(c("1;36", "│  github.com/phamlongh230-lgtm/yamtam-engine          │"));
    lines.push(c("1;36", "└─────────────────────────────────────────────────────┘"));
    lines.push(String::new());
...
pub fn render_markdown(report: &ScanReport) -> String {
    ...
    let mut lines = vec![
        "# YAMTAM Agent Audit Report".to_string(),
        String::new(),
        format!("> Generated by YAMTAM v{}  ·  {}", report.yamtam_version, report.generated_at),
    ...
    if matches!(report.risk_level.as_str(), "CRITICAL" | "HIGH") {
        lines.extend(["1. Fix all CRITICAL and HIGH findings before using AI agents".to_string(),
            "2. Re-run `yamtam audit .` to verify fixes".to_string(),
            "3. Add `yamtam audit . --fail-on high` to your CI pipeline".to_string()]);
    } else {
        lines.extend(["1. Review MEDIUM findings and apply where practical".to_string(),
            "2. Add `yamtam audit . --fail-on high` to your CI pipeline".to_string()]);
    }
    lines.extend([String::new(), "---".to_string(), String::new(),
        "*Report generated by [YAMTAM](https://github.com/phamlongh230-lgtm/yamtam-engine)*".to_string()]);
    lines.join("\n")
}
