...
/// The pure command-string judgment chain (is_rm_rf, is_git_force, and
/// everything check_command chains together) now lives in
/// `src/command_safety.rs`, a `lib.rs`-level module — not here — so the
/// exact same code the native CLI guard uses is also reachable from the
/// WASM build (`src/lib.rs`'s `mod wasm`) without a second, independently-
/// drifting copy. `main.rs`'s binary crate reaches it via `yana_rt::...`,
/// the same cross-crate pattern `yana_rt::flock_v1` already established
/// (see e.g. `src/os/health.rs`). This re-export keeps every existing
/// caller of `crate::guard::check_command` (capability::command,
/// src/mcp.rs) working unchanged.
pub use yana_rt::command_safety::check_command;
