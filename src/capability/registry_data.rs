...
pub(super) fn all_descriptors() -> Vec<CapabilityDescriptor> {
    vec![
        ...
        CapabilityDescriptor {
            ...
            description: "Propose writing (create) or replacing (overwrite) one core/config/*.json file. Same governed flow as file.write (diff shown, human approval required, backup before overwrite, verified after write) plus config-specific guardrails: path is confined to core/config/ (no subpaths), content must be valid JSON, and core-lock.json can never be targeted — that file is only regenerated by core/scripts/update-core-lock.sh after human review. Yana Studio architecture audit, Phase 4 (crate::capability::config_write).",
            access_mode: AccessMode::Mutating,
            risk_tier: RiskTier::High,
            approval: ApprovalRequirement::HumanApprovalPerCall,
            input_schema: json!({
