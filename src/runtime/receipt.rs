...
///
/// `decision_id` is generated by the caller (`capability_decision`), not
/// here — an `Allow` needs to carry the *same* id both in its own return
/// value (`AuthorityDecision::Allow { decision_id }`, so a later
/// `ExecutionReceipt` can reference it, item #4) and in this receipt, so
/// the id has to exist before this function is even called, not be
/// generated as a side effect of recording.
pub(crate) fn record(
    decision_id: &str,
    context: &TurnContext,
    capability: &str,
...
/// `yana-rt authority executions` — the reader side of item #4's
/// `ExecutionReceipt`, same "must not become invisible" reasoning as
/// `cmd_authority_receipts` above.
pub fn cmd_authority_executions(last: Option<usize>, json: bool) -> Result<()> {
    let root = std::env::current_dir().context("cannot resolve project root")?;
    let mut receipts = read_execution_receipts(&root);
    if let Some(last) = last {
        let skip = receipts.len().saturating_sub(last);
        receipts.drain(..skip);
    }
    ...
    if receipts.is_empty() {
        println!("No execution receipts recorded yet.");
        return Ok(());
    }
    for receipt in &receipts {
        let outcome_label = match receipt.outcome {
            ExecutionOutcome::Success => "SUCCESS",
            ExecutionOutcome::Error => "ERROR",
        };
        let duration_ms = (receipt.completed_at - receipt.started_at).num_milliseconds();
        println!(
            "[{}] {outcome_label:<8} subject={} capability={} turn={} decision={} ({duration_ms}ms)",
            receipt.started_at,
            receipt.subject,
            receipt.capability_id,
            receipt.turn_id,
            receipt.authority_decision_id.as_deref().unwrap_or("-"),
        );
    }
    Ok(())
}

// ── ExecutionReceipt (Authority Hardening item #4) ──────────────────────
//
// Closes the causal chain item #3 started: `Turn → Capability Proposal →
// AuthorityDecision → Capability Invocation → ExecutionReceipt →
// Observation/Result → Final Model Conclusion`. This is deliberately just
// the primitive — a schema plus one recording call site
// (`execute_approved_tool` in controller.rs) — not a graph, not a
// visualization, not a query layer. Per the milestone doc's own P3
// phasing: once every capability emits a correct receipt, a future
// Causal Evidence Graph is mostly query/visualization work over
// `authority_decision_id`/`turn_id` correlations that already exist in
// this data, not new data modeling. Building that graph now, before real
// receipts are flowing, is the exact anti-pattern the milestone doc
// warns against ("a large new subsystem before the data feeding it is
// trustworthy") — so it isn't built here.

const MAX_EXECUTION_RECEIPT_ENTRY_BYTES: usize = 64 * 1024;
...
pub(crate) struct ExecutionReceipt {
    ...
    /// Links back to the `AuthorityDecisionReceipt` that permitted this
    /// invocation (item #3). `None` only if a `ToolExecutor` impl somehow
    /// runs without going through `authorize_approved_tool` first — should
    /// not happen via `execute_approved_tool`, the one call site that
    /// records this receipt, but kept `Option` rather than assumed
    /// infallible since this is evidence, not a load-bearing invariant
    /// check (that check lives in `capability_decision` itself).
    pub authority_decision_id: Option<String>,
    pub turn_id: String,
    pub subject: String,
    /// The tool name as the provider proposed it (`call.name`) — the
    /// identifier actually available at this call site. Distinct from the
    /// canonical capability descriptor name `AuthorityDecisionReceipt`
    /// records, which `capability_decision` resolves one layer above this.
    pub capability_id: String,
    pub workspace: PathBuf,
    pub input_hash: String,
    /// `None` when the result output is empty (nothing meaningful to
    /// hash), not an error case.
...
fn execution_receipts_path(root: &Path) -> PathBuf {
    root.join(".yana-ai").join("execution-receipts.jsonl")
}
...
fn append_execution_receipt_at(root: &Path, receipt: &ExecutionReceipt) -> Result<()> {
    let path = execution_receipts_path(root);
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).with_context(|| {
            format!(
                "cannot create execution receipt directory {}",
                parent.display()
            )
        })?;
    }
    let mut line =
        serde_json::to_vec(receipt).context("cannot serialize execution receipt")?;
    line.push(b'\n');
    if line.len() > MAX_EXECUTION_RECEIPT_ENTRY_BYTES {
        bail!(
            "execution receipt exceeds {} byte limit",
            MAX_EXECUTION_RECEIPT_ENTRY_BYTES
        );
    }
    let mut options = fs::OpenOptions::new();
    options.create(true).append(true);
    ...
    let mut file = options
        .open(&path)
        .with_context(|| format!("cannot open execution receipt log {}", path.display()))?;
    if !file.metadata()?.is_file() {
        bail!(
            "execution receipt log must be a regular file: {}",
            path.display()
        );
    }
    let written = file
        .write(&line)
        .with_context(|| format!("cannot append execution receipt log {}", path.display()))?;
    if written != line.len() {
        bail!(
            "short execution receipt append at {}: wrote {written} of {} bytes",
            path.display(),
            line.len()
        );
    }
...
pub(crate) fn read_execution_receipts(root: &Path) -> Vec<ExecutionReceipt> {
    let path = execution_receipts_path(root);
    let Ok(text) = fs::read_to_string(&path) else {
        return vec![];
    };
...
/// Records one `ExecutionReceipt` for a completed capability invocation.
/// Best-effort, same reasoning as `record` above: a write failure is
/// logged to stderr and never propagates into the tool result itself.
#[allow(clippy::too_many_arguments)]
pub(crate) fn record_execution(
    context: &TurnContext,
    authority_decision_id: Option<String>,
    capability_id: &str,
    input: &str,
    output: &str,
    is_error: bool,
    ...
    let receipt = ExecutionReceipt {
        ...
        output_hash: if output.is_empty() {
            None
        } else {
    ...
    if let Err(error) = append_execution_receipt_at(&context.session.repo_root, &receipt) {
        eprintln!("[yana-rt] warning: could not record execution receipt: {error:#}");
    }
}
...
mod tests {
    ...
    /// Test helper mirroring how `capability_decision` actually constructs
    /// an Allow: mint the id once, use it both as the decision's own
    /// payload and as `record`'s `decision_id` argument.
    fn allow_decision() -> (String, AuthorityDecision) {
    ...
    fn allow_deny_and_approval_required_all_round_trip() {
        ...
        let (id, allow) = allow_decision();
        record(&id, &ctx, "command.execute", &allow, None);
        record(
        ...
        assert_eq!(receipts[0].decision_id, id);
    ...
    fn allow_via_lease_records_the_lease_id() {
        ...
        let ctx = context(&root);
        let (id, allow) = allow_decision();
        record(&id, &ctx, "command.execute", &allow, Some("abc12345".to_string()));
        let receipts = read_receipts(&root);
    ...
    fn subagent_receipt_carries_the_agent_id_and_inherits_parent_turn_id() {
        ...
        let (id, allow) = allow_decision();
        record(&id, &child, "command.execute", &allow, None);
        let receipts = read_receipts(&root);
    ...
    fn a_malformed_line_is_skipped_not_fatal_to_the_rest_of_the_log() {
        let root = temp_root();
        let ctx = context(&root);
        let (id1, allow1) = allow_decision();
        record(&id1, &ctx, "command.execute", &allow1, None);
        ...
        let (id2, allow2) = allow_decision();
        record(&id2, &ctx, "command.execute", &allow2, None);
