...
//!
//! Activity/Approval/Memory/Project preserve PR #149's reconciled layout:
//! the Session box stays fixed while Tab cycles panels backed by real
//! runtime or repository data rather than placeholders.
...
pub(super) fn render_session_box(frame: &mut Frame, area: Rect, app: &App, palette: Palette) {
    ...
    let lines = vec![
        Line::styled(
            "● Yana-rt connected",
            Style::default()
                .fg(palette.runtime_green)
                .add_modifier(Modifier::BOLD),
        ),
        Line::raw(app.provider.name().to_string()),
        Line::styled(app.model.clone(), Style::default().fg(palette.system_blue)),
        Line::raw(""),
        Line::styled(
            format!(
                "{locality} · {}",
    ...
    frame.render_widget(
        Paragraph::new(lines).block(
            Block::bordered()
                .title(" Yana-rt Session ")
                .border_style(Style::default().fg(palette.runtime_green)),
        ),
        area,
    );
}
...
pub(super) fn render_sidebar(frame: &mut Frame, area: Rect, app: &App, palette: Palette) {
    let [session_area, panel_area] =
        Layout::vertical([Constraint::Length(6), Constraint::Min(4)]).areas(area);
    render_session_box(frame, session_area, app, palette);
    match app.sidebar_tab {
        SidebarTab::Activity => render_activity_panel(frame, panel_area, app, palette),
        SidebarTab::Approval => render_approval_panel(frame, panel_area, app, palette),
        SidebarTab::Memory => render_memory_panel(frame, panel_area, app, palette),
        SidebarTab::Project => render_project_panel(frame, panel_area, app, palette),
    }
}
...
fn render_activity_panel(frame: &mut Frame, area: Rect, app: &App, palette: Palette) {
    let (status, tone) = match &app.turn {
        TurnState::Idle => ("Ready for your message", palette.user_pink),
        TurnState::Streaming(_) => ("Yana-rt is responding", palette.runtime_green),
        TurnState::AwaitingApproval(_) => ("Approval required", palette.warning),
        TurnState::ExecutingTool { .. } => ("Yana-rt tool active", palette.runtime_green),
    };
    let lines = vec![
        Line::styled(
            status,
...
fn render_approval_panel(frame: &mut Frame, area: Rect, app: &App, palette: Palette) {
...
fn render_memory_panel(frame: &mut Frame, area: Rect, app: &App, palette: Palette) {
    ...
    frame.render_widget(
        List::new(items).block(panel_block(title, palette.violet)),
        area,
    );
}

fn render_project_panel(frame: &mut Frame, area: Rect, app: &App, palette: Palette) {
    ...
    frame.render_widget(
        Paragraph::new(lines).block(panel_block(
            SidebarTab::Project.label().to_string(),
            palette.system_blue,
        )),
        area,
    );
