...
Yana (You Are Not Alone)는 메모리, 워크플로우, 보안 제어(Safety Controls) 및 지속적인 학습을 통해 사용자와 AI 에이전트가 더욱 신뢰할 수 있는 방식으로 협업하도록 돕는 에이전트 운영체제(Agent OS)입니다.
여기서 "You Are Not Alone"이라는 이름은 매우 광범위하고 전문적인 의미를 담고 있습니다. 이는 더 이상 인간의 외로움을 뜻하는 것이 아니라, 운영체제(OS)와 AI 에이전트 간의 신뢰할 수 있는 동행을 의미합니다. 당신은 독립적으로 작동하는 AI 에이전트가 길을 잃지 않고, 루프에 갇히지 않으며(Stuck), 항상 올바른 방향으로 나아갈 수 있도록 엄격하면서도 안전한 '감독관'을 만들어가고 있습니다.
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/📜_규칙-69-b96b80?style=flat-square" />
  <img src="https://img.shields.io/badge/🪝_훅-55-b78f3d?style=flat-square" />
...
**Yana AI**는 AI 코딩 도구를 위한 개인용 에이전트 운영체제입니다 — 런타임 보안 후크, 메모리 계층, 101개의 전문 에이전트, 1,989개의 스킬, 그리고 위험한 AI 행동을 실행 전에 가로채는 Rust 런타임으로 구성됩니다.

**Claude Code**, **Cursor**, **Windsurf**, **Antigravity**, **Kiro**, **OpenCode**, **Zed**, **Gemini**, **GitHub Copilot**, **Aider** 등 다양한 도구와 함께 동작합니다.


> **v0.43.0의 새 기능:** **yana-rt가 이제 브라우저에서 실행됩니다** — wasm-bindgen을 통한 WebAssembly 빌드로 브라우저 확장, VS Code, Node.js(`npm install yana-rt`)에 위험 명령어 가드를 제공합니다. 퍼블리시 파이프라인 완전 자동화 — 태그 푸시 시 npm, PyPI, crates.io 모두 자동 배포됩니다. 에이전트 수가 101개로 정확히 조정되었습니다. Rule 70 추가: 컨텍스트 충실성 법칙 — 제공된 데이터를 학습 데이터보다 우선시합니다.
>
> **2026-07-01 수정:** 위의 "101개로 조정" 표기 자체도 이전에는 틀렸습니다 (기존 162라는 숫자는 저장소의 다른 어떤 파일과도 일치하지 않았습니다 — ARCHITECTURE.md는 95, 실제 문서 사이트 데이터는 93). 이 페이지의 모든 수치는 이제 손으로 입력하는 대신 `core/scripts/generate-stats.py`가 파일시스템에서 직접 생성합니다.
...
> **101개의 에이전트란?** 동시에 실행되는 101개의 AI 모델이 아닙니다 — 라우팅과 작업 조직을 위해 미리 정의된 전문 역할(보안, 프론트엔드, 백엔드, 테스트, 학습, 일상 비서…)입니다. 평상시 사용에서는 현재 작업에 필요한 에이전트만 활성화되며, 대부분의 요청은 단일 모델과 단일 에이전트 경로만 사용합니다.
>저장소 메트릭으로부터 생성됨
마지막 업데이트: 2026-06-30
---
...
| | |
|---|---|
| 🧩 스킬 | **1,989**개의 워크플로 스킬 정의 |
| 🤖 에이전트 | **101**개의 전문 에이전트 |
| 📜 안전 규칙 | **69**개의 강제 규칙 |
| 🪝 후크 | **55**개의 사전/사후 실행 후크 |
| ⚡ 슬래시 명령 | **166**개 |
...
```
core/
├── hooks/          # 50개의 PreToolUse / PostToolUse / Stop 후크
├── rules/          # 67개의 강제 규칙 (보안, 정확성, UI, git)
├── scripts/        # safe-run.sh, verify-core-lock.sh, secure-logger.sh
├── gates/          # truth_gate.md, action_gate.md
├── agents/         # 101개의 전문 에이전트 정의
├── skills/         # 1,989개의 SKILL.md 파일
├── config/
...
- **주권자 권한** — 인간이 101개 에이전트 전체를 즉시 동결할 수 있음
...
1,989개의 스킬은 프론트엔드, 백엔드, AI/LLM, 보안, Kubernetes, WebAssembly, DevOps, 데이터베이스, 테스트 등을 포괄합니다. 코딩 외 용도를 위한 새로운 에이전트 페르소나 2종: 학습(`hoc-tap`)과 일상 생산성(`daily-assistant`).
...
- **skill** → 1,989개 항목의 인덱스와 매칭, 정확한 스킬 에이전트로 디스패치
...
저장소 스캔으로 발견된 스킬 정의 1,989개
2026-06-30 기준 측정, 8,438개 파일
첫 커밋: 2026-05-17
