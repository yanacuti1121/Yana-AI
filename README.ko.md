...
파괴적인 git 작업, 워크스페이스 밖의 `rm`, 인터넷 콘텐츠를 bash로 파이프하는 행위, 검증되지 않은 패키지 설치를 Rust 런타임(`yana-rt`)이 뒷받침하는 58개의 에이전트 훅으로 막습니다. 그 아래에는 101개의 전문 에이전트, 2,016개의 스킬, CI에서 830가지 방식으로 검사되는 71개의 시행 규칙이 있습니다.
...
| | |
|---|---|
| 🧩 Skills | **2,016**개 워크플로 스킬 정의 |
| 🤖 Agents | **101**개 전문 에이전트 |
| 📜 Safety rules | **71**개 시행 규칙 |
| 🪝 Hooks | **58**개 실행 전/후 훅 |
