...
| --- | --- | --- |
| [OpenHands](https://github.com/All-Hands-AI/OpenHands) (현재 "Agent Canvas" — 자체 문서에 이 UI는 "에이전트 행동을 직접 실행하거나... sandbox를 제공하는" 책임이 없다고 명시되어 있음; 그 계층은 [`OpenHands/software-agent-sdk`](https://github.com/OpenHands/software-agent-sdk)에 있음) | 여러 coding-agent 백엔드(Claude Code, Codex, Gemini, 모든 ACP 에이전트)를 오케스트레이션하는 셀프호스팅 컨트롤 센터 | 에이전트 오케스트레이션 중심 — 에이전트를 실행하는 컨트롤 센터이지, 어떤 에이전트가 무엇을 실행해도 되는지에 대한 authority calculus가 아님 |
