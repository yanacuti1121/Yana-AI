...
Yana AI is not an IDE, not a terminal, and not just an agent pack.  
...
Yana AI Blackbox OS closes that gap.
...
**Output:**
```
.yana-ai/blackbox/session-2026-05-25T14-32-01.jsonl   ← machine-readable
.yana-ai/blackbox/session-2026-05-25T14-32-01.md      ← human-readable timeline
```
...
**Output:**
```
.yana-ai/evidence/session-2026-05-25T14-32-01-graph.json
```
...
**Output:**
```
.yana-ai/autopsy/2026-05-25-readme-drift.md
...
1. Generated by autopsy → saved to `.yana-ai/immune/pending/`
...
Two layers: engine-level (part of Yana AI core, shared across repos) and project-level (per-repo, session-specific data).

### Engine-level additions (in `yana-ai/`)
```
core/
  constitution/
...
### Project-level (in each repo using Yana AI, gitignored by default)
```
.yana-ai/
...
9f. IMMUNE UPDATED      .yana-ai/immune/pending/ ← new entry
```
...
Add to Yana AI README under a new section "Blackbox OS":

---

### Yana AI Blackbox OS: flight recorder for AI coding agents

Most AI coding tools execute actions and move on. Yana AI records what happened, verifies what was claimed, and learns from what went wrong.
...
**Agent Autopsy** — When a session fails, Yana AI produces a structured post-mortem: what went wrong, which gate was missing, what rule and test would have blocked it.
...
- `.yana-ai/blackbox/` directory creation on session start
...
| Full system | yana-ai-blackbox-os | Yana AI Blackbox OS |
