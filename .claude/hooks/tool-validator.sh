...
#
# Bypass: YAMTAM_TOOL_VALID_BYPASS=1
# Test seam: TOOL_VALID_TEST_INPUT="<json>"

set -uo pipefail

[[ "${YAMTAM_TOOL_VALID_BYPASS:-}" == "1" ]] && exit 0
...
if ! printf '%s' "$TOOL_NAME" | grep -qE "^($KNOWN_TOOLS)$"; then
  warn "⚠️  Tool Validator [L1.5]: Unknown tool '${TOOL_NAME}' requested. This tool is not in the YAMTAM known-tools allowlist. Verify this tool exists and is expected before allowing. Reference: core/hooks/tool-validator.sh"
fi
...
if [[ "$TOOL_NAME" == "Bash" ]]; then
  ...
  if printf '%s' "$CMD" | grep -qP '\x00' 2>/dev/null || \
    ...
    deny "Blocked [L1.5 Tool Validator]: Null byte (\\x00) detected in Bash command. Null byte injection is a known attack vector for bypassing string-based filters. Bypass: YAMTAM_TOOL_VALID_BYPASS=1"
  fi
  ...
  if [[ -n "$TIMEOUT_VAL" ]] && ! printf '%s' "$TIMEOUT_VAL" | grep -qE '^[0-9]+$'; then
    deny "Blocked [L1.5 Tool Validator]: Invalid 'timeout' field in Bash tool input — must be a non-negative integer, got: '${TIMEOUT_VAL}'. Bypass: YAMTAM_TOOL_VALID_BYPASS=1"
  fi
fi
...
if [[ "$TOOL_NAME" == "Write" || "$TOOL_NAME" == "Edit" || "$TOOL_NAME" == "MultiEdit" ]]; then
  ...
  if [[ -n "$FILE_PATH" ]]; then
    ...
    if printf '%s' "$FILE_PATH" | grep -qE '^(/etc/(passwd|shadow|sudoers|hosts|crontab|ssh)|/root/|/proc/|/sys/)'; then
      deny "Blocked [L1.5 Tool Validator]: Write to sensitive system path '${FILE_PATH}' is not allowed. System files must not be written by AI agents. Bypass: YAMTAM_TOOL_VALID_BYPASS=1"
    fi

    # Path traversal: ../../ sequences targeting outside project root
    if printf '%s' "$FILE_PATH" | grep -qE '(^|/)\.\.(/|$)'; then
      deny "Blocked [L1.5 Tool Validator]: Path traversal detected in '${FILE_PATH}'. Sequences like '../..' can escape the project root. Use absolute paths within the project or project-relative paths. Bypass: YAMTAM_TOOL_VALID_BYPASS=1"
    fi

    # Absolute paths outside project root (warn, not block — may be legitimate)
    PROJECT_ROOT="${CLAUDE_PROJECT_DIR:-$(pwd)}"
    if [[ "$FILE_PATH" == /* ]] && [[ "$FILE_PATH" != "$PROJECT_ROOT"* ]]; then
      warn "⚠️  Tool Validator [L1.5]: Write/Edit target '${FILE_PATH}' is outside the project root '${PROJECT_ROOT}'. Confirm this is intentional. Bypass: YAMTAM_TOOL_VALID_BYPASS=1"
    fi
  fi
fi
...
if [[ "$TOOL_NAME" == "WebFetch" ]]; then
  ...
  if [[ -n "$URL" ]]; then
    ...
    if ! printf '%s' "$URL" | grep -qE '^https?://'; then
      deny "Blocked [L1.5 Tool Validator]: WebFetch URL '${URL}' must use http:// or https:// scheme. Other schemes (file://, ftp://, data://) are not allowed. Bypass: YAMTAM_TOOL_VALID_BYPASS=1"
    fi
    ...
    if printf '%s' "$HOST" | grep -qE \
      ...
      deny "Blocked [L1.5 Tool Validator]: WebFetch to private/loopback address '${HOST}' is blocked (SSRF guard). Fetching internal network addresses may expose internal services. Bypass: YAMTAM_TOOL_VALID_BYPASS=1"
    fi

    # Block metadata services (cloud SSRF)
    if printf '%s' "$HOST" | grep -qE \
      '^(169\.254\.169\.254|metadata\.google\.internal|169\.254\.170\.2|fd00:ec2::254)$'; then
      deny "Blocked [L1.5 Tool Validator]: WebFetch to cloud metadata endpoint '${HOST}' is blocked. Accessing instance metadata services can expose cloud credentials. Bypass: YAMTAM_TOOL_VALID_BYPASS=1"
    fi
  fi
fi
