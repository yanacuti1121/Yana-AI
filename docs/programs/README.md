...
| J | Universal Capability Runtime | `Draft` — Phase 1: 0 Open Question. Scope M=4→5 + Claude Code, tất cả qua MCP Server thay translator-per-engine, không đánh đổi an toàn (cơ chế chặn bắt buộc giữ nguyên). `core/config/mcp-whitelist.json` được tạo. Phase 3 (2 sơ đồ luồng) + Interfaces (schema `check_command`, dựa trên MCP spec thật) + Phase 4 (pipeline có nhánh lỗi/timeout/audit) đều xong 2026-07-24. Phase 5 Readiness chưa bắt đầu | `PROGRAM-J-SKELETON.md` |
