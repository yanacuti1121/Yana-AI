...
| K | Yana OS (tên/chữ cái tạm, chưa xác nhận) — AI Agent Operating System: local-first execution + governance platform quản lý agent lifecycle/identity/capabilities/sessions/policies/sandboxing/memory/scheduling/observability, xây trên nền `yana-rt` (không thay thế). Lấy cảm hứng kiến trúc từ `cloudflare-os`, không phụ thuộc hạ tầng Cloudflare | **ADS Phases 3–15 complete for the approved slice** (2026-08-11): authoritative ownership fixed, canonical strict accounting and read-only aggregate doctor implemented/reviewed/benchmarked/documented. Supervisor, scheduler, vault, managed-agent authorization, schema migration, Windows mutation and kernel resource control remain BLOCKED at 40–55% readiness. | `PROGRAM-K-YANA-OS-SKELETON.md` · `PROGRAM-K-PHASE-2-CAPABILITY-INVENTORY.md` · `PROGRAM-K-PHASES-3-15.md` |
