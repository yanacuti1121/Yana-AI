...
| K | Yana OS (tên/chữ cái tạm, chưa xác nhận) — AI Agent Operating System: local-first execution + governance platform quản lý agent lifecycle/identity/capabilities/sessions/policies/sandboxing/memory/scheduling/observability, xây trên nền `yana-rt` (không thay thế). Lấy cảm hứng kiến trúc từ `cloudflare-os`, không phụ thuộc hạ tầng Cloudflare | `Draft` — Phase 0 (Vision/Problem/Relationship/Scope) đã có nội dung thật do anh Tâm trả lời trực tiếp (2026-08-09), Phase 1 Specification chưa bắt đầu. Anh Tâm đã khoá scope: chỉ định kiến trúc ở Phase 0, không đề xuất implementation/migration/refactor cho tới khi ADR + boundaries được duyệt | `PROGRAM-K-YANA-OS-SKELETON.md` |
