...
YAMTAM ENGINE is a standalone agent operating system that wraps Claude Code (and other AI coding tools) with runtime safety enforcement, memory, agents, and verification. It lives **outside** product repositories and intercepts AI actions before they cause harm.

```
User
  └─► Claude Code (or Cursor / Aider / Copilot / Gemini / DeepSeek / Qwen)
            └─► [YAMTAM HOOKS] ── PreToolUse / PostToolUse / Stop
                      ├── PASS  → tool executes
                      └── BLOCK → action denied, reason logged
```

The engine is **deployed by installing its runtime pack** (`yamtam-engine-vX.Y.Z.zip`) into the `.claude/` directory of any target project, or via the Claude Code plugin system. The source of truth for all components lives in this repository's `core/` tree.
...
Emergency bypass variables: `YAMTAM_DEPLOY_APPROVED=1`, `YAMTAM_SCOPE_OK=1`, `YAMTAM_TRUTH_GATE_BYPASS=1`.
...
When the release pack (`yamtam-engine-vX.Y.Z.zip`) is installed into a target project's `.claude/` directory, it exposes:
...
Installation via: `/plugin install phamlongh230-lgtm/yamtam-engine`

---

### 6. Cross-Engine Adapters: `adapters/`

YAMTAM gate enforcement is delivered to non-Claude engines via adapter configs generated by `switch-engine.sh`:

| Engine | Adapter | Enforcement tier |
|--------|---------|-----------------|
| Claude Code | Native hooks (L0–L5) | Full runtime blocking |
| Cursor | `.cursor/rules/yamtam-hard-enforcement.mdc` | Hard enforcement via safe-run.sh proxy |
...
| File | Purpose |
|------|---------|
| `docs/SEPARATION.md` | Boundary: YAMTAM vs product repos |
...
| `docs/yamtam-blackbox-os.md` | Blackbox OS direction document |
| `docs/yamtam-system-map.html` | Interactive system map |
...
2. blast-radius        Cap destructive scope (score 0–5; ≥4 blocks without YAMTAM_IRREVERSIBLE_OK)
...
`docs/yamtam-blackbox-os.md` defines the long-term architectural direction. **Status: concept-draft, not yet implemented. Roadmap target: v1.9.0+.**

The Blackbox OS adds five modules on top of the current gate layer:

| Module | Purpose | Roadmap |
|--------|---------|---------|
| **Agent Flight Recorder** | Capture replayable session timeline (intent → plan → action → evidence → claim) in `.yamtam/blackbox/session-*.jsonl` | v1.8.x |
...
*YAMTAM ENGINE v1.7.3 · Apache 2.0 License · Maintained by Vũ Văn Tâm*
